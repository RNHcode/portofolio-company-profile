...
if($token == '' or $email == ''){
    ...
    $sql1 = "select * from where email = '$email' and token_ganti_password = '$token'";
...
if(isset($_POST['submit'])){
    ...
    if($password == '' or $konfirmasi_password == ''){
        $err .= "Silahkan masukan password serta konfrimasi password";
    }elseif($konfirmasi_password != $password){
        $err .= "Konfirmasi password tidak sesuai dengan password";
    }elseif(strlen($password) < 6){
        $err .= "Jumlah karakter yang diperboleehkan untuk password minimal 6 karakter";
    }
    if(empty($err)){
        $sql1 = "update members set token_ganti_password = '', password=md5($password) where email = '$email'";
        mysqli_query($koneksi, $sql1);
        $sukses = "Password berhasil di ganti. Silahkan <a href='" . url_dasar() . "/login.php'>login</a>";
    }
}
?>
<?php if ($err) {
